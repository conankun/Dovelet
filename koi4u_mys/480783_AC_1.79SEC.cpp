...
struct keyset {
	int ind,value;
	bool operator<(const keyset &k) const {
...
int main() {
	...
	for(i=1;i<=N;i++) {
		...
		pre[i].index=i;
		gogo[i].value = pre[i].h;
		gogo[i].ind = i;
	}
	stable_sort(gogo+1,gogo+1+N);
	stable_sort(pre+1,pre+1+N);
	int size=1;
	int lank=0;
	for(i=1;i<=N;i++) {
		if(gogo[i].value != gogo[i-1].value) lank++;
		key[gogo[i].ind]=lank;
	}
	...
	for(i=1;i<=N;i++) {
		...
		int ls=size+1,rs=size+key[pre[i].index]-1;
		bool ok=true;
		int ans=0,index=0;
		while(ls<=rs&&ok) {
			if(ls==rs) ok=false;
		...
		back[i] = index;
		qu[++t].ind = i;
		qu[t].limit = size+key[pre[i].index];
		qu[t].value = ans+1;
		if(ans+1 > mx) {
			mx=ans+1;
			mxindex = i;
