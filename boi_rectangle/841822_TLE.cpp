		...
		for(j=i+1;j<=c_pair;j++) {
			if(slope[i].length == slope[j].length) {
				if(slope[i].point1.x+slope[i].point2.x == slope[j].point1.x+slope[j].point2.x && slope[i].point1.y+slope[i].point2.y==slope[j].point1.y+slope[j].point2.y) {//if have identical midpoint
						//calculate the area
							deltax=max(slope[i].point1.x,max(slope[i].point2.x,max(slope[j].point1.x,slope[j].point2.x)))-min(slope[i].point1.x,min(slope[i].point2.x,min(slope[j].point1.x,slope[j].point2.x)));
							deltay=max(slope[i].point1.y,max(slope[i].point2.y,max(slope[j].point1.y,slope[j].point2.y)))-min(slope[i].point1.y,min(slope[i].point2.y,min(slope[j].point1.y,slope[j].point2.y)));
							totalrect=deltax*deltay;
							one1 = abs((slope[i].point1.x-slope[j].point1.x)*(slope[i].point1.y-slope[j].point1.y));
							one2 = abs((slope[i].point1.x-slope[j].point2.x)*(slope[i].point1.y-slope[j].point2.y));
							two1 = abs((slope[i].point2.x-slope[j].point1.x)*(slope[i].point2.y-slope[j].point1.y));
							two2 = abs((slope[i].point2.x-slope[j].point2.x)*(slope[i].point2.y-slope[j].point2.y));
