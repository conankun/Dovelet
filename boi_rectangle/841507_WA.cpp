	...
	for(i=1;i<=c_pair;i++) {
		for(j=i+1;j<=c_pair;j++) {
			if(fabs(slope[i].length - slope[j].length) < 0.0000000001) {
				...
				long long one1 = abs((slope[i].point1.x-slope[j].point1.x)*(slope[i].point1.y-slope[j].point1.y)/2);
				long long one2 = abs((slope[i].point1.x-slope[j].point2.x)*(slope[i].point1.y-slope[j].point2.y)/2);
				long long two1 = abs((slope[i].point2.x-slope[j].point1.x)*(slope[i].point2.y-slope[j].point1.y)/2);
				long long two2 = abs((slope[i].point2.x-slope[j].point2.x)*(slope[i].point2.y-slope[j].point2.y)/2);
				long long ara=totalrect-one1-one2-two1-two2;
